...
class AddressDefaultWidgetTest extends WebTestBase {
  ...
  function testFields() {
    ...
    foreach (['US', 'FR', 'CN'] as $country) {
      ...
      $edit[$fieldName . '[0][country_code]'] = $country;
      $this->drupalPostAjaxForm($this->nodeAddUrl, $edit, $fieldName . '[0][country_code]');
  ...
  function testSubdivisions() {
    ...
    $edit[$fieldName . '[0][country_code]'] = $country;
    $this->drupalPostAjaxForm($this->nodeAddUrl, $edit, $fieldName . '[0][country_code]');
    $this->assertOptions($fieldName . '[0][administrative_area]', array_keys($administrativeAreas), 'All administrative areas for country ' . $country . ' are present.');

    // Confirm the presence and format of the locality dropdown.
    $edit = [];
    $edit[$fieldName . '[0][administrative_area]'] = $administrativeArea;
    $this->drupalPostAjaxForm(NULL, $edit, $fieldName . '[0][administrative_area]');
    $this->assertResponse(200);
    ...
    $this->assertOptions($fieldName . '[0][locality]', array_keys($localities), 'All localities for administrative area ' . $administrativeAreas[$administrativeArea] . ' are present.');
    ...
    $this->assertOptions($fieldName . '[0][dependent_locality]', array_keys($dependentLocalities), 'All dependent localities for locality ' . $localities[$locality] . ' are present.');
  }
  ...
  function testClearValues() {
    ...
    $this->drupalGet('node/' . $node->id() . '/edit');
    $this->assertFieldByName($fieldName . '[0][country_code]', 'US');
    $this->assertFieldByName($fieldName . '[0][administrative_area]', 'US-CA');
    $this->assertFieldByName($fieldName . '[0][locality]', 'Mountain View');
    $this->assertFieldByName($fieldName . '[0][postal_code]', '94043');
    ...
    $this->assertFieldByName($fieldName . '[0][country_code]', 'CN', 'Country changed to CN');
    $this->assertFieldByName($fieldName . '[0][administrative_area]', '', 'Field administrative_area has been cleared');
    $this->assertFieldByName($fieldName . '[0][locality]', '', 'Field locality has been cleared');
    $this->assertFieldByName($fieldName . '[0][dependent_locality]', '', 'Field dependent_locality has been cleared');
    $this->assertFieldByName($fieldName . '[0][postal_code]', '', 'Field postal_code has been cleared.');
  }
